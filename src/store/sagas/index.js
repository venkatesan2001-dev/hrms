...
function* fetchAllocationsWorker(action) {
  try {
    ...
    const { data } = yield call(axios.get, `/api/payroll/${userId}`)
...
function* createAllocationWorker(action) {
  try {
    const { data } = yield call(axios.post, '/api/payroll', action.payload)
...
function* fetchPayrunsWorker() {
  try {
    const { data } = yield call(axios.get, '/api/payrun')
...
function* generatePayrunWorker(action) {
  try {
    const { data } = yield call(axios.post, '/api/payrun/generate', action.payload)
...
function* processPayrunWorker(action) {
  try {
    const id = action.payload
    const { data } = yield call(axios.post, `/api/payrun/${id}/process`)
